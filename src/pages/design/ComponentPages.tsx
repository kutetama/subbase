...
function Preview({ kind }: { kind: DocKey }) {
  ...
  if (kind === 'button') { const rows=[['Primary','bg-[#1C6BFF] text-white','bg-[#244BD7] text-white'],['Secondary','bg-[#EBF1FD] text-[#1C6BFF]','bg-[#E1EBFF] text-[#244BD7]'],['Tertiary','border border-[#E2E5EB] bg-white text-[#222]','border border-[#E2E5EB] bg-[#EBEEF1] text-[#222]'],['Critical','bg-[#EE4700] text-white','bg-[#EE4700] text-white'],['Ghost','bg-transparent text-[#222]','bg-[#F2F4F8] text-[#222]'],['Text','text-[#1C6BFF]','text-[#244BD7]']] as const; return <div className="grid gap-5">{rows.map(([name,normal,pressed])=><div key={name} className="grid grid-cols-[110px_repeat(3,1fr)] items-center gap-3"><strong className="text-sm">{name}</strong><button className={`${baseButton} ${normal}`}>Normal</button><button className={`${baseButton} ${pressed}`}>Pressed, Hover</button><button disabled className={baseButton}>Disabled</button></div>)}<div className="flex flex-wrap gap-3 border-t border-line pt-5">{['Dialog','Subtle','Function','Utility','Detail','Textfield'].map(x=><button key={x} className={`${baseButton} border border-line bg-white text-[#222]`}>{x}</button>)}</div></div>; }
  if (kind === 'label-helper') return <div className="grid max-w-md gap-7"><label className="grid gap-2 text-sm font-semibold">필수 입력 레이블 <span className="font-normal text-[#EE4700]">* 필수 항목입니다.</span><input className="h-11 rounded-[6px] border border-[#E2E5EB] px-3 font-normal" placeholder="입력 가이드 문구" /></label><label className="grid gap-2 text-sm font-semibold">레이블<input className="h-11 rounded-[6px] border border-[#EE4700] px-3 font-normal" value="잘못된 값" readOnly /><span className="font-normal text-[#EE4700]">에러 문구</span></label></div>;
  if (kind === 'text-field') return <div className="grid max-w-2xl grid-cols-2 gap-5">{[['Placeholder','플레이스홀더','border-[#E2E5EB]'],['Normal','입력된 값','border-[#E2E5EB]'],['Focused','입력 중인 값','border-[#1C6BFF]'],['Error','입력된 값','border-[#EE4700]'],['Readonly','Disabled or Read only','border-transparent bg-[#FBFBFD] text-[#C5C8CE]']].map(([label,value,style])=><label key={label} className="grid gap-2"><code className="text-xs text-fg-subtle">{label}</code><input className={`h-11 rounded-[6px] border px-3 outline-none ${style}`} placeholder={label==='Placeholder'?value:undefined} value={label==='Placeholder'?undefined:value} readOnly/></label>)}<label className="grid gap-2"><code className="text-xs text-fg-subtle">Standalone</code><input className="h-11 rounded-[6px] border border-[#E2E5EB] px-3" value="Standalone" readOnly/></label></div>;
  if (kind === 'text-area') return <div className="grid grid-cols-3 gap-4"><label className="grid gap-2"><code className="text-xs text-fg-subtle">Placeholder</code><textarea className="h-32 resize-none rounded-[6px] border border-[#E2E5EB] p-3" placeholder="플레이스홀더"/></label><label className="grid gap-2"><code className="text-xs text-fg-subtle">Normal</code><textarea className="h-32 resize-none rounded-[6px] border border-[#E2E5EB] p-3" value="입력된 값 입력된 값\n입력된 값" readOnly/></label><label className="grid gap-2"><code className="text-xs text-fg-subtle">Focused</code><textarea className="h-32 resize-none rounded-[6px] border border-[#1C6BFF] p-3 outline-none" value="입력 중인 값\n입력 중인 값" readOnly/></label></div>;
  if (kind === 'selection-control') return <div className="grid gap-7"><div className="flex flex-wrap gap-8"><InputRadio valuePair={{id:'radio',name:'라디오 버튼'}} checked={checked} disabled={false} onChange={()=>setChecked(true)}/><InputCheckbox valuePair={{id:'check',name:'서비스 이용약관 동의'}} checked={checked} disabled={false} onChange={({checked:next})=>setChecked(next)}/><InputCheckbox valuePair={{id:'limited',name:'Checkbox (Limited) · 3'}} checked={false} disabled={false} onChange={()=>{}}/><label className="flex items-center gap-3"><Switch on={checked} onChange={setChecked}/><span>응답 실패 효과음</span></label></div><div className="flex flex-wrap gap-8 opacity-50"><InputRadio valuePair={{id:'disabled',name:'Disabled'}} checked={false} disabled onChange={()=>{}}/><InputCheckbox valuePair={{id:'disabled-check',name:'Disabled'}} checked disabled onChange={()=>{}}/><Switch on={false} disabled onChange={()=>{}}/></div></div>;
  if (kind === 'dropdown') { const list=<div className="w-56 rounded-[6px] border border-[#E2E5EB] bg-white p-2 shadow-[0_0_16px_rgba(107,110,116,.16)]">{['옵션 이름 1','옵션 이름 2','옵션 이름 3','옵션 이름 4','옵션 이름 5'].map(x=><button key={x} className="block w-full rounded px-3 py-2 text-left hover:bg-[#F2F4F8]">{x}</button>)}</div>; return <div className="grid grid-cols-2 gap-8"><div><code className="mb-2 block text-xs text-fg-subtle">Selectbox</code><select className="h-11 w-56 rounded-[6px] border border-[#E2E5EB] bg-white px-3"><option>플레이스홀더</option><option>선택한 값</option></select></div><div><code className="mb-2 block text-xs text-fg-subtle">Selectbox List</code>{list}</div><div><code className="mb-2 block text-xs text-fg-subtle">Custom</code><button className="h-11 w-56 rounded-[6px] border border-primary bg-white px-3 text-left">설정된 값</button></div><div><code className="mb-2 block text-xs text-fg-subtle">Custom List</code>{list}</div></div>; }
  if (kind === 'date-picker') return <DatePicker value={date} defaultMonth={new Date(2024,0,1)} onChange={setDate}/>;
  if (kind === 'slider') return <div className="grid gap-10"><Slider value={value} label onChange={setValue}/><RangeSlider value={range} onChange={setRange}/></div>;
  if (kind === 'chip') return <div className="flex flex-wrap gap-3"><Chip label="필터" selected/><Chip label="옵션" type="option" selected/><Chip label="입력한 값" type="input" onRemove={()=>{}}/><Chip label="Disabled" disabled/></div>;
  if (kind === 'menu') return <div className="max-w-md border border-line"><AppBar title="페이지 타이틀" action={<span>•••</span>}/><div className="ml-auto mr-3 w-56 rounded-[6px] bg-white p-2 shadow-[0_0_16px_rgba(107,110,116,.16)]">{[1,2,3,4].map(x=><button key={x} className="block w-full rounded px-3 py-2.5 text-left hover:bg-[#F2F4F8]">더보기 메뉴 {x}</button>)}</div></div>;
  if (kind === 'snackbar') return <div className="grid max-w-xl gap-4"><div><code className="mb-2 block text-xs text-fg-subtle">Normal</code><Snackbar message="저장되었습니다."/></div><div><code className="mb-2 block text-xs text-fg-subtle">Action</code><Snackbar message="디바이스 상태가 변경되었습니다." actionLabel="취소"/></div><div><code className="mb-2 block text-xs text-fg-subtle">Flexible</code><Snackbar message={'디바이스 연결에 실패하였습니다.\n다시 시도해주세요.'} actionLabel="닫기"/></div></div>;
  if (kind === 'tab') { const groups=[['Primary',['탭 이름','이름2']],['Secondary',['주간','월간']],['3',['최근 들은','많이 들은','좋아요']],['4',['이름','이름2','이름3','이름4']],['5',['최근 통화','연락처','114','좋아요','설정']],['Scrollable',["Editor’s Pick",'최근 들은','많이 들은','좋아요']]] as const; return <div className="grid gap-7">{groups.map(([label,tabs])=><div key={label}><code className="mb-2 block text-xs text-fg-subtle">{label}</code><div className="flex max-w-full overflow-x-auto border-b border-[#E2E5EB]">{tabs.map((x,i)=><button key={x} onClick={()=>setSelected(x)} className={`h-12 shrink-0 px-6 ${(selected===x||(!selected&&i===0))?'border-b-2 border-[#1C6BFF] font-semibold text-[#1C6BFF]':'text-[#5F646F]'}`}>{x}</button>)}</div></div>)}</div>; }
  if (kind === 'pagination') return <div className="grid gap-7"><div><code className="mb-2 block text-xs text-fg-subtle">Numbered</code><div className="flex items-center gap-2"><button className="size-9">‹</button>{[1,2,3,4].map(x=><button key={x} className={`size-9 rounded-full ${x===1?'bg-[#1C6BFF] text-white':''}`}>{x}</button>)}<button className="size-9">›</button></div></div><div><code className="mb-3 block text-xs text-fg-subtle">Dots</code><div className="flex gap-2">{[0,1,2,3].map(x=><span key={x} className={`size-2 rounded-full ${x===0?'bg-[#1C6BFF]':'bg-[#E2E5EB]'}`}/>)}</div></div></div>;
  if (kind === 'badge') return <div className="grid gap-7"><div className="flex items-center gap-5"><Badge name="19" className="size-[22px] justify-center rounded-full bg-primary text-white"/><Badge name="뱃지 레이블" className="rounded-full bg-primary-bg text-primary"/><Badge name="Rectangle" className="rounded-none bg-neutral-lightGray text-fg-muted"/><Badge name="New" className="bg-primary text-white"/><Badge name="Error" className="bg-error-bg text-error"/><Badge name="닷" className="rounded-full bg-neutral-lightGray text-fg-muted"/></div><div className="flex items-end gap-5"><Badge name="Large" className="h-7 bg-caution-bg text-caution"/><Badge name="Medium" className="bg-success-bg text-success"/><Badge name="Small" className="h-[18px] bg-error-bg text-error"/></div></div>;
  if (kind === 'rating') return <div className="grid gap-3">{[5,4,3,2,1,0].map(x=><div key={x} className="flex items-center gap-5"><code className="w-4 text-xs text-fg-subtle">{x}</code><Rating value={x} readOnly/></div>)}<div className="mt-3 border-t border-line pt-4"><Rating value={rating} onChange={setRating}/></div></div>;
  if (kind === 'progress-indicator') return <div className="flex items-center gap-12"><div><code className="mb-3 block text-xs text-fg-subtle">Standalone</code><Spinner/></div><div><code className="mb-3 block text-xs text-fg-subtle">Label</code><Spinner label="전송 중"/></div></div>;
  if (kind === 'chart') { const charts=[['Group Bar','group-bar'],['Bar','bar'],['Bar & Line','bar-line'],['Group Line','group-line'],['Lollipop','lollipop'],['Stacked Line','stacked-line'],['Donut','donut'],['Gauge','gauge']] as const; return <div className="grid grid-cols-2 gap-5">{charts.map(([label,type])=><figure key={type} className="rounded-control border border-line p-4"><figcaption className="mb-2 text-sm font-semibold">{label}</figcaption><Chart type={type} className="h-[220px] w-full"/></figure>)}</div>; }
  if (kind === 'divider') return <div className="grid gap-8"><div><p>리스트 제목</p><Divider className="my-4"/><p className="text-sm text-[#5F646F]">상세 설명 상세 설명</p></div><div className="flex h-12 items-center gap-5"><span>왼쪽</span><Divider direction="vertical"/><span>오른쪽</span></div></div>;
  if (kind === 'dialog') return <div className="grid grid-cols-2 gap-6">{[['Alert','이용 동의가 철회되었습니다.'],['Confirm','동의 철회 하시겠습니까?'],['Confirm with Desc.','일부 디바이스는 서비스를 지원하지 않습니다.'],['Confirm Scrollable','회원 탈퇴 시 서비스에 설정된 모든 데이터가 삭제되며 복구할 수 없습니다.']].map(([type,message])=><div key={type} className="rounded-[8px] bg-white p-6 shadow-[0_0_30px_rgba(107,110,116,.2)]"><code className="text-xs text-fg-subtle">{type}</code><h3 className="mt-3 font-semibold">{type==='Alert'?'알림':'동의 철회'}</h3><p className={`mt-3 text-sm leading-6 ${type==='Confirm Scrollable'?'max-h-20 overflow-y-auto':''}`}>{message}</p><div className="mt-8 flex justify-end gap-6">{type!=='Alert'&&<button className="text-[#9397A1]">취소</button>}<button className="font-semibold text-[#1C6BFF]">확인</button></div></div>)}</div>;
  if (kind === 'section-message') return <div className="grid max-w-xl gap-3"><SectionMessage message="로그인해주세요."/><SectionMessage type="ghost" message="연결중입니다."/><SectionMessage type="error" message="연결에 실패했습니다." onAction={()=>{}}/></div>;
  return <div className="grid max-w-2xl gap-8"><Description>상세 설명 상세 설명 상세 설명 상세 설명 상세 설명</Description><Description>상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명</Description><Description tone="subtle">상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명 상세 설명</Description></div>;
}

export function ComponentDocPage({ kind }: { kind: DocKey }) {
  const doc = docs[kind];
