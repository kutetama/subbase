...
export function ContentPage({ kind }: { kind: keyof typeof contentDocs }) {
...
export function PatternPage({ kind }: { kind: PatternKind }) {
...
export function BestPracticePage({ kind }: { kind: keyof typeof practices }) {
  const [title, node]=practices[kind];
  let preview;
  if(kind==='content') preview=<div className="mx-auto max-w-md overflow-hidden border border-line bg-white"><AppBar title="페이지명" action={<button className="px-3 text-primary">버튼 레이블</button>}/><Subheader title="리스트 제목"/><ListItem title="볼륨" description="캡션 캡션 캡션" action="›"/><SectionBlock title="로그인해주세요."/><div className="border-t border-line p-4 text-sm text-fg-muted">스크롤 했을 경우 상단 구조와 정보 위계를 유지합니다.</div></div>;
...
const SectionBlock = ({ title }: { title: string }) => <div className="mx-4 my-3 rounded-control border border-line bg-semantic-bg px-4 py-3 text-sm text-fg-muted">{title}</div>;
